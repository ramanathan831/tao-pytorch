...
"""Test cases for Deformable DETR quantization functionality."""
...
def dataset_config_with_calib(temp_dataset):
    ...
    data_config.train_data_sources = [
        {"image_dir": temp_dataset["tmp_dir"], "json_file": temp_dataset["json_file"]}
    ]
...
def test_dataset_config_has_quant_calibration_data_sources():
    """Test that DDDatasetConfig has quant_calibration_data_sources field."""
    config = DDDatasetConfig()
    assert hasattr(config, 'quant_calibration_data_sources'), (
        "DDDatasetConfig should have quant_calibration_data_sources field"
    )
...
def test_calib_dataloader_setup(dataset_config_with_calib):
    ...
    dm = ODDataModule(dataset_config_with_calib)
...
@pytest.mark.cv_unit
def test_calib_dataloader_raises_without_data_sources():
    """Test that calibration stage raises error if no data sources provided."""
    data_config = OmegaConf.structured(DDDatasetConfig())
    data_config.quant_calibration_data_sources = None
    data_config.batch_size = TEST_BATCH_SIZE
    data_config.workers = 0
    data_config.num_classes = 4

    dm = ODDataModule(data_config)
    # Setup with calibration stage should raise an error if sources are None
    with pytest.raises(ValueError, match="quant_calibration_data_sources.image_dir must be provided"):
        dm.setup(stage="calibration")
