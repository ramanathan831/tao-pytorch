...
class DINOv3TrainExpConfig(NVDINOv2TrainExpConfig):
    ...
    custom attention, and adds a ``distributed_strategy`` selector. The nvdinov2
    default (Lightning ``'auto'`` -> single-device / DDP) is unchanged; FSDP
    (FULL_SHARD) is opt-in and is what enables high-resolution and the larger
    ViT-L / ViT-H+ backbones, where DDP's full per-GPU replication does not fit.
    The ``DINOV3_STRATEGY`` env var, kept for the de-risking smokes, overrides this
    field when set.
    """
