...
"""Quantize a Mask Grounding DINO model using the configured backend.

This script loads a trained Mask Grounding DINO checkpoint, prepares the calibration data loader
...
@monitor_status(name="Mask-Grounding-DINO", mode="quantize")
def main(cfg: ExperimentConfig) -> None:
    ...
    logger.info("Starting Mask Grounding DINO quantization")

    # Build the Lightning model and extract the underlying nn.Module
    logger.debug("Loading Mask Grounding DINO checkpoint")
    ...
    logger.info("Mask Grounding DINO quantization completed successfully")
