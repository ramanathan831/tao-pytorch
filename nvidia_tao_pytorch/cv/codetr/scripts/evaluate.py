...
def run_experiment(experiment_config, key):
    ...
    if model_path.endswith('.tlt') or model_path.endswith('.pth'):
        # Build model and load checkpoint with key mapping
        ...
        if result.missing_keys:
            logging.info("Missing keys (expected for collab heads / buffers): %d", len(result.missing_keys))
        if result.unexpected_keys:
            logging.warning("Unexpected keys: %s", result.unexpected_keys[:10])
...
@monitor_status(name="CoDETR", mode="evaluate")
def main(cfg: ExperimentConfig) -> None:
    """Run CoDETR evaluation."""
