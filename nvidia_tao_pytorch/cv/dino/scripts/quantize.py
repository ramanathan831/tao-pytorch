...
"""Quantize a DINO model using the configured backend.

This script loads a trained DINO checkpoint, prepares the calibration data loader
...
@monitor_status(name="DINO", mode="quantize")
def main(cfg: ExperimentConfig) -> None:
    ...
    logger.info("Starting DINO quantization")

    # Build the Lightning model and extract the underlying nn.Module
    logger.debug("Loading DINO checkpoint")
    if not cfg.quantize.model_path.endswith(".onnx"):
        pl_model = DINOPlModel.load_from_checkpoint(
    ...
    logger.info("DINO quantization completed successfully")
