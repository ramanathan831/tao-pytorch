...
"""Block module for the VGGT model."""
...
class Block(nn.Module):
    ...
    Args:
        ...
        ffn_bias (bool): Whether to use bias in the FFN. Default is True.
        drop (float): Dropout rate for the attention. Default is 0.0.
        attn_drop (float): Dropout rate for the attention. Default is 0.0.
        init_values (float): Initial value for the LayerScale. Default is None.
        drop_path (float): Drop path rate. Default is 0.0.
        act_layer (Callable[..., nn.Module]): Activation layer. Default is nn.GELU.
        norm_layer (Callable[..., nn.Module]): Normalization layer. Default is nn.LayerNorm.
        attn_class (Callable[..., nn.Module]): Attention class. Default is Attention.
        ffn_layer (Callable[..., nn.Module]): FFN layer. Default is Mlp.
...
def drop_add_residual_stochastic_depth(
    x: Tensor, residual_func: Callable[[Tensor], Tensor],
    ...
    Args:
        x (Tensor): Input features of shape (B, N, C).
        residual_func (Callable[[Tensor], Tensor]): Residual function.
        sample_drop_ratio (float): Sample drop ratio. Default is 0.0.
        pos (Tensor): Positional embedding of shape (B, N, C). Default is None.
...
def get_branges_scales(x, sample_drop_ratio=0.0):
    ...
    Returns:
        Tuple[Tensor, float]: Tuple of brange and residual scale factor.
    """
    b, _, _ = x.shape
    sample_subset_size = max(int(b * (1 - sample_drop_ratio)), 1)
    brange = (torch.randperm(b, device=x.device))[:sample_subset_size]
...
def add_residual(x, brange, residual, residual_scale_factor, scaling_vector=None):
    """
    Add residual to the input features.

    Args:
        x (Tensor): Input features of shape (B, N, C).
        brange (Tensor): Brange of shape (N,).
        residual (Tensor): Residual of shape (N, C).
        residual_scale_factor (float): Residual scale factor.
        scaling_vector (Tensor): Scaling vector. Default is None.
...
def get_attn_bias_and_cat(x_list, branges=None):
    ...
    Args:
        ...
        branges (List[Tensor]): List of branges of shape (N,). Default is None.

    Returns:
        Tuple[Tensor, Tensor]: Tuple of attention bias and cat tensors.
    """
...
def drop_add_residual_stochastic_depth_list(
    ...
    Args:
        x_list (List[Tensor]): List of input features of shape (B, N, C).
        residual_func (Callable[[Tensor, Any], Tensor]): Residual function.
        sample_drop_ratio (float): Sample drop ratio. Default is 0.0.
        scaling_vector (Tensor): Scaling vector. Default is None.
...
class NestedTensorBlock(Block):
    ...
    def forward(self, x_or_x_list):
        ...
        Args:
            x_or_x_list (Tensor or List[Tensor]): Input features or list of input features.

        Returns:
            Tensor or List[Tensor]: Output features or list of output features.
        """
