...
from nvidia_tao_pytorch.cv.classification_pl.utils.utils_vis import save_with_text_overlay
...
class ClassifierPlModel(TAOLightningModule):
    ...
    def __init__(self, experiment_spec, export=False):
        ...
        if self.binary:
            ...
            for topk in self.model_config.head.topk:
                train_acc[f"train_acc_{topk}"] = Accuracy(task="multiclass", num_classes=self.n_class, top_k=topk)
                val_acc[f"val_acc_{topk}"] = Accuracy(task="multiclass", num_classes=self.n_class, top_k=topk)
    ...
    def _build_criterion(self):
        ...
        assert self.model_config.head.loss.type in ["CrossEntropyLoss"], "Only CrossEntropyLoss is supported."
        if self.model_config.head.loss.type == 'CrossEntropyLoss':
            self.criterion = Cross_Entropy(binary=self.binary, label_smoothing=self.model_config.head.loss.label_smooth_val)
        else:
    ...
    def configure_optimizers(self):
        ...
        if self.lr_policy == 'linear':
            ...
            if self.lr_policy_params is not None:
                ...
                gamma = 0.1
            # args.lr_decay_iters
            scheduler = lr_scheduler.StepLR(self.optimizer_G, step_size=step_size, gamma=gamma)
        elif self.lr_policy == 'CosineAnnealingLR':
            scheduler = lr_scheduler.CosineAnnealingLR(self.optimizer_G, T_max=200, eta_min=0)
        else:
    ...
    def _write_prediction_row(self, writer, name, class_scores, pred_label, pred_score, with_gt_label, gt_label=None):
    ...
    def _visualize_predictions(self, out, batch, batch_idx, with_gt_label=False):
        ...
        """
        out = torch.nn.functional.softmax(out, dim=1)  # Apply softmax to get probabilities
        file_name = expand_path(os.path.join(self.vis_dir, "result.csv"))  # Get the file path
        ...
        with open(file_name, 'a') as f:  # Open the file in append mode
            ...
            for i in range(len(batch['name'])):  # Iterate over the batch size
                ...
                pred_score, pred_label_idx = torch.max(out[i], 0)  # Get the max score and corresponding label index
                pred_label = self.class_names[pred_label_idx.item()]  # Map index to class name

                if with_gt_label:  # Include ground truth label if required
                    gt_label = self.class_names[batch['class'][i].item()]
                    self._write_prediction_row(writer, batch["name"][i], each_class_score, pred_label, pred_score.item(), with_gt_label, gt_label)
                else:
                    self._write_prediction_row(writer, batch["name"][i], each_class_score, pred_label, pred_score.item(), with_gt_label)
                    # for pred_score, only keep 4 decimal points
                    write_out_argument.append([pred_label_idx.item(), round(pred_score.item(), 3), pred_label])
        ...
        if not with_gt_label:
            ...
            for i in range(len(batch['name'])):
                ...
                img_path = expand_path(os.path.join(self.vis_dir, "visualize", filename))
                # save_with_text_overlay(img, write_out_argument[i], img_path, imgs_size[i])
                save_with_text_overlay(batch['name'][i], write_out_argument[i], img_path)
